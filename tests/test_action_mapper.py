...
class TestActionMapper(unittest.TestCase):
    def test_execute_command_uses_first_token_for_action(self):
        action = build_action_id("execute_command", {"command": "rm test.txt"})
        self.assertEqual("execute_command.rm", action)

    def test_execute_command_date_maps_to_date_action(self):
    ...
    def test_non_command_tool_returns_prefix(self):
        ...
        self.assertEqual("weather_operations.get_weather", action)

    def test_unknown_operation_returns_unknown(self):
        action = build_action_id("weather_operations", {"operation": "unknown_op"})
